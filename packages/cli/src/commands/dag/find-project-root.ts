 ...
 * workspace root and is not present in intermediate packages or dist
 * directories — so using generic markers like `package.json` would cause
 * the walk-up to stop prematurely at the first nested package.
 *
...
export const findProjectRoot = (startDir?: string): string => {
  ...
  // Walk up until we find the agents/ directory or hit the filesystem root
  while (true) {
...
/**
 * Throw a descriptive error when `projectRoot` does not contain an `agents/`
...
export const validateProjectRoot = (projectRoot: string, explicitFlag: boolean): void => {
  ...
  const hint = explicitFlag
    ? `--project "${projectRoot}" does not contain an agents/ directory.`
