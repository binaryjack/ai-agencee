 ...
 * 
 * Rolls back changes by checking out files from a specific git commit.
 */
...
export class GitCommitStrategy implements IRollbackStrategy {
  validate(snapshot: Snapshot): string | undefined {
    if (!snapshot.commitHash) {
      return 'No commit hash in snapshot';
    }
    return undefined;
  }
  ...
  async undoFile(
    ...
    const { exitCode, stderr } = await executeGitCommand(
      ['checkout', snapshot.commitHash!, '--', filePath],
      snapshot.projectRoot,
    );
