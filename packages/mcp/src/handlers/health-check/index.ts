...
const checkMcpServer = async (_projectRoot: string): Promise<HealthCheckResult> => {
  ...
  try {
    ...
    clearTimeout(timer)
    if (res.ok) return { name, status: 'ok', message: 'MCP server reachable at localhost:3001' }
...
const checkModelRouter = async (projectRoot: string): Promise<HealthCheckResult> => {
...
const checkTechRegistry = async (_projectRoot: string): Promise<HealthCheckResult> => {
...
const checkAgenceeInit = async (projectRoot: string): Promise<HealthCheckResult> => {
  const name = 'agencee-init'
  if (fs.existsSync(path.join(projectRoot, '.agencee'))) {
    return { name, status: 'ok', message: '.agencee/ initialised' }
  }
  return { name, status: 'error', message: '.agencee/ not found', fix: 'ai-kit init' }
}

const checkAgentFiles = async (projectRoot: string): Promise<HealthCheckResult> => {
...
const checkCloudApi = async (_projectRoot: string): Promise<HealthCheckResult> => {
...
const checkCodernic = async (projectRoot: string): Promise<HealthCheckResult> => {
  const name = 'codernic'
...
export const runHealthCheck = async (projectRoot: string): Promise<DoctorReport> => {
